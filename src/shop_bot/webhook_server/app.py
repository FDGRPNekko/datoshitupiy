...
from shop_bot.data_manager.database import (
    ...
    add_new_key, add_new_key_unified, add_key_host, get_balance, adjust_user_balance, get_referrals_for_user,
    get_user, get_key_by_email, get_key_by_subscription_token,
    search_users,
    get_all_promo_codes, create_promo_code, delete_promo_code,
)
...
def create_webhook_app(bot_controller_instance):
    ...
    @flask_app.route('/sub/<subscription_token>')
    def subscription_aggregator(subscription_token):
        """Агрегатор подписки: один URL возвращает конфиги со всех хостов (как remnawave)."""
        key = get_key_by_subscription_token(subscription_token)
        if not key:
            return '', 404
        try:
            expiry = key.get('expiry_date')
            if expiry:
                exp_dt = datetime.fromisoformat(str(expiry).replace('Z', '')) if isinstance(expiry, str) else expiry
        ...
        try:
            key_id = key['key_id']
            configs = asyncio.run(xui_api.get_aggregated_connection_strings(key_id))
        except Exception as e:
            logger.warning(f"subscription_aggregator key_id={key.get('key_id')}: {e}")
            configs = []
        if not configs:
            return '', 404
        body = '\n'.join(configs)
    ...
    @login_required
    def users_page():
        # Список не выводим — только поиск
        common_data = get_common_template_data()
        return render_template('users.html', users=[], **common_data)

    @flask_app.route('/users/search')
    @login_required
    def users_search_route():
        q = (request.args.get('q') or '').strip()
        users = []
        if q:
            users = search_users(q, limit=100)
            for user in users:
    ...
    def promos_page():
        ...
        try:
            ...
            promos = []
        common_data = get_common_template_data()
        return render_template('admin_promos.html', promos=promos, **common_data)
    ...
    def add_promo_route():
        ...
        if not code:
            flash('Введите код промокода.', 'danger')
            return redirect(url_for('promos_page'))
        try:
            discount_percent = float(request.form.get('discount_percent') or 0)
            days_bonus = int(request.form.get('days_bonus') or 0)
            max_activations = int(request.form.get('max_activations') or 0)
            min_purchase_rub = float(request.form.get('min_purchase_rub') or 0)
        except ValueError:
        ...
        if valid_from:
            try:
                valid_from = datetime.fromisoformat(valid_from).isoformat()
            except Exception:
                valid_from = None
        if valid_until:
            try:
        ...
        pid = create_promo_code(
            ...
            min_purchase_rub=min_purchase_rub,
        )
        if pid:
            flash(f'Промокод «{code}» создан.', 'success')
        else:
            flash('Не удалось создать промокод (возможно, такой код уже есть).', 'danger')
        return redirect(url_for('promos_page'))

    @flask_app.route('/admin/promos/<int:promo_id>/delete', methods=['POST'])
    @login_required
    ...
    def create_key_route():
        ...
        if not key_email:
            flash('Укажите email ключа.', 'danger')
            return redirect(request.referrer or url_for('admin_keys_page'))
        try:
            results = asyncio.run(xui_api.create_or_update_key_on_all_hosts(
                base_email=key_email,
                expiry_timestamp_ms=expiry_ms or None,
            ))
        except Exception as e:
            logger.error(f"Не удалось создать ключ на панелях: {e}")
            flash('Не удалось создать ключ на панелях. Проверьте доступность XUI.', 'danger')
            return redirect(request.referrer or url_for('admin_keys_page'))
        if not results:
            flash('Не удалось создать ключ ни на одном хосте.', 'danger')
            return redirect(request.referrer or url_for('admin_keys_page'))

        expiry_ms = results[0].get('expiry_timestamp_ms') or expiry_ms
        new_id, subscription_token = add_new_key_unified(user_id, key_email, expiry_ms)
        if not new_id:
            flash('Ошибка при сохранении ключа в БД.', 'danger')
            return redirect(request.referrer or url_for('admin_keys_page'))
        for r in results:
            add_key_host(new_id, r['host_name'], r['client_uuid'], r['email'])

        connection_string = xui_api.get_unified_subscription_link(subscription_token)
        flash('Ключ добавлен (все серверы в одной подписке).', 'success')
        ...
        try:
            ...
            if bot and new_id:
                text = (
                    '🔐 Ваш ключ готов!\n'
                    'Выдан администратором. Все серверы в одной подписке.\n'
                )
                if connection_string:
                    cs = html_escape.escape(connection_string)
                    text += f"\nПодключение:\n<code>{cs}</code>"
                loop = current_app.config.get('EVENT_LOOP')
                if loop and loop.is_running():
    ...
    def create_key_ajax_route():
        ...
        if not key_email:
            return jsonify({"ok": False, "error": "key_email required"}), 400
        ...
        try:
            ...
            logger.error(f"create_key_ajax_route: {e}")
        ...
        try:
            bot = _bot_controller.get_bot_instance()
            if bot and new_id:
                text = '🔐 Ваш ключ готов! Выдан администратором. Все серверы в одной подписке.\n'
                if connection_string:
                    text += f"\nПодключение:\n<pre><code>{html_escape.escape(connection_string)}</code></pre>"
                loop = current_app.config.get('EVENT_LOOP')
                if loop and loop.is_running():
                    asyncio.run_coroutine_threadsafe(
                        bot.send_message(chat_id=user_id, text=text, parse_mode='HTML', disable_web_page_preview=True),
            ...
            if action == 'reply':
                if not message:
                    ...
                    try:
                        ...
                        if bot and loop and loop.is_running() and forum_chat_id and thread_id:
                            text = f"💬 Ответ Поддержки по вашему тикету #{ticket_id}:\n\n{message}"
                            asyncio.run_coroutine_threadsafe(
                ...
                if ticket.get('status') != 'closed' and set_ticket_status(ticket_id, 'closed'):
                    ...
                    try:
                        ...
                        user_chat_id = ticket.get('user_id')
                        if bot and loop and loop.is_running() and user_chat_id:
                            text = f"✅ Ваш тикет #{ticket_id} был закрыт службой поддержки. Вы можете создать новое обращение при необходимости."
    ...
    def add_host_route():
        ...
        try:
            synced = asyncio.run(xui_api.sync_keys_to_new_host(host_name))
            if synced:
                flash(f"Хост '{host_name}' добавлен. Синхронизировано ключей: {synced}.", 'success')
            else:
                flash(f"Хост '{host_name}' успешно добавлен.", 'success')
        except Exception as e:
            logger.warning(f"sync_keys_to_new_host after add_host: {e}")
            flash(f"Хост '{host_name}' добавлен. Синхронизация ключей не выполнена.", 'warning')
        return redirect(url_for('settings_page', tab='hosts'))
