...
def update_or_create_client_on_panel(api: Api, inbound_id: int, email: str, days_to_add: int | None = None, target_expiry_ms: int | None = None) -> tuple[str | None, int | None, str | None]:
    try:
        ...
        # Шаг 1: Ищем существующего клиента
        client_index = -1
        existing_client_id = None
        for i, client in enumerate(inbound_to_modify.settings.clients):
            client_email = getattr(client, 'email', '').strip().lower()
            if client_email == email.strip().lower():
                client_index = i
                existing_client_id = str(getattr(client, 'id', ''))  # FIX: str и default ''
                break
        
        # Определяем новый expiry (без изменений)
        ...
        if client_index != -1:
            ...
            existing_client = inbound_to_modify.settings.clients[client_index]
            client_uuid = existing_client_id or str(uuid.uuid4())  # FIX: fallback UUID если ID пустой
            existing_client.id = client_uuid  # Убедимся в ID
            ...
            # Sub token: ищем или генерируем (упрощённо, без setattr на неизвестные поля)
            try:
                ...
                if not sub_token_existing:
                    ...
                    # Пробуем setattr, но если fail — игнор (не критично для update)
                    for attr in ("subId", "subscription", "sub_id"):
                        try:
                            setattr(existing_client, attr, client_sub_token)
                        except Exception as set_e:
                            logger.debug(f"Не удалось установить {attr}: {set_e}")  # FIX: debug лог
                else:
                    client_sub_token = sub_token_existing
            except Exception as token_e:
                logger.warning(f"Ошибка с sub_token для {email}: {token_e}")
                import secrets
                client_sub_token = secrets.token_hex(12)

            logger.debug(f"Обновляем клиента {email}: ID={client_uuid}, expiry={new_expiry_ms}, sub_token={client_sub_token}")  # FIX: добавлен debug для отладки

        else:
            # Создаём нового (без изменений, но с debug)
            ...
            logger.debug(f"Создаём нового клиента {email}: UUID={client_uuid}, sub_token={client_sub_token}")

        # FIX: Всегда обновляем через inbound.update (стабильно работает)
        api.inbound.update(inbound_id, inbound_to_modify)
        logger.info(f"Клиент {email} успешно {'обновлён' if client_index != -1 else 'создан'} на inbound {inbound_id}")

        return client_uuid, new_expiry_ms, client_sub_token

    except ValueError as ve:
        # Обработка UNIQUE (fallback на уникальный email, без изменений)
        error_msg = str(ve)
        if "UNIQUE constraint failed: client_traffics.email" in error_msg:
            logger.warning(f"Дубликат email {email} обнаружен. Генерирую уникальный...")
            unique_email = f"{email.rsplit('@', 1)[0]}-{uuid.uuid4().hex[:8]}@{email.rsplit('@', 1)[1]}"
            return update_or_create_client_on_panel(api, inbound_id, unique_email, days_to_add, target_expiry_ms)
        raise
    except Exception as e:
        logger.error(f"Ошибка в update_or_create_client_on_panel для {email}: {e}", exc_info=True)  # FIX: добавлен email в лог для ясности
