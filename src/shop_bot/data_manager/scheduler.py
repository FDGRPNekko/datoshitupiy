        ...
        try:
            ...
            for db_key in keys_in_db:
                ...
                if server_client:
                    ...
                    logger.warning(f"Scheduler: Ключ '{key_email}' для хоста '{host_name}' не найден на сервере. Не удаляю с БД. (Пропускаю)")
                    #database.update_key_status_from_server(key_email, None)
                    #total_affected_records += 1
                    continue

                if clients_on_server:
                    logger.debug(f"Scheduler: Пропускаю обработку осиротевших клиентов на '{host_name}' (отключено).")
