...
from shop_bot.data_manager.database import (
    get_user, add_new_key, add_new_key_unified, add_key_host, get_key_hosts,
    get_user_keys, update_user_stats,
    register_user_if_not_exists, get_next_key_number, get_key_by_id,
    update_key_info, update_key_expiry, set_trial_used, set_terms_agreed, get_setting, get_all_hosts,
...
def get_user_router() -> Router:
    ...
    async def trial_period_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        await callback.answer()
        await process_trial_key_creation_unified(callback.message)

    async def process_trial_key_creation_unified(message: types.Message):
        """Создать пробный ключ на всех хостах, одна ссылка подписки."""
        user_id = message.chat.id
        trial_days = int(get_setting("trial_duration_days") or "3")
        await message.edit_text(f"Создаю для вас бесплатный ключ на {trial_days} дн. (все серверы в одной подписке)...")
        ...
        try:
            ...
            results = await xui_api.create_or_update_key_on_all_hosts(
                base_email=candidate_email,
            ...
            expiry_ms = results[0]["expiry_timestamp_ms"]
            new_key_id, subscription_token = add_new_key_unified(user_id, candidate_email, expiry_ms)
            if not new_key_id:
                await message.edit_text("❌ Ошибка сохранения ключа.")
                return
            for r in results:
                add_key_host(new_key_id, r["host_name"], r["client_uuid"], r["email"])

            connection_string = xui_api.get_unified_subscription_link(subscription_token)
            new_expiry_date = datetime.fromtimestamp(expiry_ms / 1000)
            final_text = get_purchase_success_text("готов", get_next_key_number(user_id) - 1, new_expiry_date, connection_string)
            try:
    ...
    async def buy_new_key_handler(callback: types.CallbackQuery):
        ...
        if not hosts:
            ...
            return
        # Единый ключ: тарифы берём с первого хоста (ключ создаётся на всех хостах)
    ...
    async def extend_key_handler(callback: types.CallbackQuery):
        ...
        if host_name in (None, "", "all"):
            hosts = get_all_hosts()
            host_name = hosts[0].get("host_name") if hosts else None
        if not host_name:
            await callback.message.edit_text("❌ Ошибка: Нет доступных серверов для продления.")
            return

        plans = get_plans_for_host(host_name)
        if not plans:
            await callback.message.edit_text("❌ Нет настроенных тарифов для продления.")
            return
    ...
    async def show_payment_options(message: types.Message, state: FSMContext):
        ...
        if data.get('promo_applied_price') is not None:
            final_price = Decimal(str(data['promo_applied_price']))
            message_text = (
                f"🎟 Промокод применён.\n"
                f"<b>К оплате: {float(final_price):.2f} RUB</b>\n\n"
            ) + CHOOSE_PAYMENT_METHOD_MESSAGE
        ...
        promo_applied = data.get('promo_code') is not None and data.get('promo_applied_price') is not None
        kw = dict(
        ...
        try:
            await message.edit_text(message_text, reply_markup=keyboards.create_payment_method_keyboard(**kw))
        except TelegramBadRequest:
            await message.answer(message_text, reply_markup=keyboards.create_payment_method_keyboard(**kw))
        await state.set_state(PaymentProcess.waiting_for_payment_method)

    @user_router.callback_query(PaymentProcess.waiting_for_payment_method, F.data == "enter_promo")
    async def enter_promo_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        await state.set_state(PaymentProcess.waiting_for_promo)

    @user_router.message(PaymentProcess.waiting_for_promo, F.text)
    async def process_promo_message(message: types.Message, state: FSMContext):
        ...
        plan = get_plan_by_id(data.get('plan_id'))
        if not plan:
            await message.answer("❌ Ошибка: тариф не найден. Начните выбор заново.")
            await state.clear()
            return
        base_price = float(plan['price'])
        months = int(plan['months'])
        ok, msg, final_price, extra_days = validate_and_apply_promo(code, base_price, months)
        if not ok:
            await message.answer(f"❌ {msg}")
            return
        await state.update_data(
            promo_code=code,
            promo_applied_price=final_price,
            promo_extra_days=extra_days,
        )
        extra_info = ""
        if extra_days:
            extra_info = f"\n+ {extra_days} дн. к подписке."
        await message.answer(f"✅ {msg}\nК оплате: {final_price:.2f} RUB.{extra_info}")
        await show_payment_options(message, state)
    ...
    async def create_yookassa_payment_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        if data.get('promo_applied_price') is not None:
            price_rub = Decimal(str(data['promo_applied_price']))
        ...
        try:
            ...
            payment_payload = {
                ...
                "metadata": {
                    ...
                    "promo_extra_days": str(data.get('promo_extra_days') or "0"),
                }
            }
            if receipt:
                payment_payload['receipt'] = receipt
    ...
    async def create_yoomoney_payment_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        metadata = {
            ...
            "promo_extra_days": data.get('promo_extra_days') or 0,
        }
    ...
    async def create_stars_invoice_handler(callback: types.CallbackQuery, state: FSMContext, bot: Bot):
        ...
        if data.get('promo_applied_price') is not None:
            price_rub = Decimal(str(data['promo_applied_price']))
        elif user_data and user_data.get('referred_by') and user_data.get('total_spent', 0) == 0:
            try:
                discount_percentage = Decimal(get_setting("referral_discount") or "0")
            except Exception:
                discount_percentage = Decimal("0")
    ...
    async def create_ton_invoice_handler(callback: types.CallbackQuery, state: FSMContext):
        ...
        metadata = {
            ...
            "promo_code": data.get('promo_code') or "", "promo_extra_days": data.get('promo_extra_days') or 0,
        }
        create_pending_transaction(payment_id, user_id, float(price_rub), metadata)
...
async def process_successful_payment(bot: Bot, metadata: dict):
    ...
    try:
        ...
        try:
            promo_extra_days = int(metadata.get('promo_extra_days', 0) or 0)
        except (TypeError, ValueError):
            promo_extra_days = 0
        days_to_add = int(months * 30) + promo_extra_days
        ...
        if action == "new":
            ...
            if not results:
                await processing_message.edit_text("❌ Не удалось создать ключ на панелях.")
                return
            expiry_ms = results[0]["expiry_timestamp_ms"]
            key_id, subscription_token = add_new_key_unified(user_id, candidate_email, expiry_ms)
            if not key_id:
                await processing_message.edit_text("❌ Ошибка сохранения ключа.")
                return
            for r in results:
                add_key_host(key_id, r["host_name"], r["client_uuid"], r["email"])
            result = {
                "connection_string": xui_api.get_unified_subscription_link(subscription_token),
            ...
            existing_key = get_key_by_id(key_id)
            key_hosts_list = get_key_hosts(key_id)
            if not key_hosts_list:
                # Старый ключ без key_hosts — продлеваем на одном хосте
                r = await xui_api.create_or_update_key_on_host(
                    host_name=existing_key.get("host_name") or host_name,
                    email=candidate_email,
                    days_to_add=int(months * 30),
                )
                if not r:
                    await processing_message.edit_text("❌ Не удалось продлить ключ в панели.")
                    return
                update_key_info(key_id, r["client_uuid"], r["expiry_timestamp_ms"])
                result = r
            else:
                # Единый ключ: продлеваем на всех хостах
                try:
                    expiry_dt = datetime.fromisoformat(str(existing_key["expiry_date"]).replace("Z", ""))
                except Exception:
                    expiry_dt = datetime.now()
                new_expiry_dt = expiry_dt + timedelta(days=days_to_add)
                new_expiry_ms = int(new_expiry_dt.timestamp() * 1000)
                for kh in key_hosts_list:
                    await xui_api.create_or_update_key_on_host(
                        host_name=kh["host_name"],
                        email=kh["key_email"],
                        expiry_timestamp_ms=new_expiry_ms,
                    )
                update_key_expiry(key_id, new_expiry_ms)
                result = {
                    "connection_string": xui_api.get_unified_subscription_link(existing_key.get("subscription_token") or ""),
                    "expiry_timestamp_ms": new_expiry_ms,
                }
                if not result["connection_string"]:
                    details = await xui_api.get_key_details_from_host(existing_key)
                    if details:
                        result["connection_string"] = details.get("connection_string", "")
        else:
            result = None
        ...
        if metadata.get('promo_code'):
            try:
                increment_promo_used(metadata['promo_code'])
            except Exception as e:
                logger.warning(f"increment_promo_used: {e}")
